...
const AddClass = () => {
  ...
  const addTask= () => {
    ...
    const response = axios.post("http://localhost/academic/admin/tasks/push_tasks.php", { grade: params.grade, section: params.section, category: type, task: title, expire_date: expiration})
      .then((res) => (res.data))
      .then((data) => {
        if (data.status == 'OK') {
