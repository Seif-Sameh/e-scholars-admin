...
const Students = ({ grade, section }) => {
    ...
    const requestStudents = () => {
        const response = axios.post("http://localhost/academic/admin/students/retrieve_students.php", { grade: grade, section: section })
            .then((res) => res.data)
            .then((data) => {
                if (data.status == 'OK') {
                    setFound(data.found)
    ...
    const activateStudent = (name) => {
        const response = axios.post("http://localhost/academic/admin/students/activate_student.php", { name: name, grade: grade, section: section })
            .then((res) => res.data)
            .then((data) => {
                if (data.status == 'OK') {
                    requestStudents()
    ...
    const deactivateStudent = (name) => {
        const response = axios.post("http://localhost/academic/admin/students/deactivate_student.php", { name: name, grade: grade, section: section })
            .then((res) => res.data)
            .then((data) => {
                if (data.status == 'OK') {
                    requestStudents()
    ...
    const deleteStudent = (name) => {
        const response = axios.post("http://localhost/academic/admin/students/remove_student.php", { name: name, grade: grade, section: section })
            .then((res) => res.data)
            .then((data) => {
                if (data.status == 'OK') {
                    requestStudents()
    ...
    const submitAttendance = () => {
        axios.post("http://localhost/academic/admin/attendance/push_attendance.php", {
            grade: grade,
            section: section,
                                                        ...
                                                        <td className='text-end border-y-[6px] border-white rounded-r-lg px-2 text-lg max-md:text-base max-sm:text-sm'>
                                                            ...
                                                            </div>
                                                            {
                                                                (toggleOptions && selected == index) && (
                                                                    <div className={`absolute w-[180px] z-[90]  bg-white right-1 top-full p-3 flex-col gap-2 divide-y-2 text-lg rounded-md shadow-md`}>
                                                                        {
                                                                            item[1] == 'active' ?
                                                                                (
