...
const Notifications = () => {
    ...
    const pushNotification = () => {
        if (selectingAll) {
            const response = axios.post("http://localhost/academic/admin/notifications/push_notification.php", { request_data: [{ message: message, grade: 'all', section: 'all', expire_date: expirationDate }] })
                .then((res) => (res.data))
                .then((data) => {
                    if (data.status == 'OK') {
                        setExpirationDate('')
            ...
            const response = axios.post("http://localhost/academic/admin/notifications/push_notification.php", { request_data: requestData })
                .then((res) => {
                    setExpirationDate('')
                    setSelectDate(false)
                    setSelectingGrades(false)
